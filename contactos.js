var newContactAddBtn = document.querySelector('.addContactBtn'),
...
newContactAddBtn.addEventListener('click', ()=> {
    isEdit = false
...
uploadimg.onchange = function(){
    if(uploadimg.files[0].size < 1000000){   // 1MB = 1000000
        ...
        alert("Este archivo es muy grande")
    }
...
function displayIndexBtn(){
    ...
    pagination.innerHTML = '<button onclick="prev()" class="prev">Previo</button>'
...
function showInfo(){
    ...
    if(getData.length > 0){
        ...
        userInfo.innerHTML = `<tr class="employeeDetails"><td class="empty" colspan="11" align="center">No hay contactos agregados</td></tr>`
        table.style.minWidth = "1400px"
    }
}
...
function editInfo(id, pic, fname, lname, Age, City, Position, Salary, SDate, Email, Phone){
    ...
    popupForm.classList.add('active')
    popupFooter.style.display = "block"
    modalTitle.innerHTML = "Actualizar el formulario"
    submitBtn.innerHTML = "Actualzar"
    formInputFields.forEach(input => {
...
function deleteInfo(index){
    if(confirm("¿Seguro que desear eliminar este contacto?")){
